...
export const useMainStore = defineStore({
  ...
  actions: {
    ...
    get_surveys(dates) {
      ...
      const testing = false;
      // avoid large calls during testing
      if (!testing) {
        try {
          let survey_refs = dates.map((date) => doc(db, "survey", "daily", date, this.user.uid));
